...
angular.module('mainApp')
    .controller("statisticsController",['$scope', '$rootScope', '$http', '$state', '$translate', 'dialogs', 'BASE_CONSTS',
        ...
        var calculateStat = function(){
            ...
            $http.post(BASE_CONSTS.ip + 'statistics', selectedBudgets)
                .then(
                    function (response) {
        ...
        view.loadStatistics = function () {
            // view.yearTotalBudget= ('$' + view.totalBudget.YEAR.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")).replace("$-", "-$");
            // view.yearTotalRemainder = ('$' + view.totalRemainder.YEAR.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")).replace("$-", "-$");
            // view.weekTotalRemainder = ('$' + view.totalRemainder.WEEK.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")).replace("$-", "-$");
            // view.fortnightTotalRemainder = ('$' + view.totalRemainder.FORTNIGHT.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")).replace("$-", "-$");
            // view.monthTotalRemainder = ('$' + view.totalRemainder.MONTH.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")).replace("$-", "-$");
