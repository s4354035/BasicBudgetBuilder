...
//        return  jdbcTemplate.query(sqlQueryForCategories, rowMapper, userId, categoryId);
    }
...
//        String str =  Joiner.on(",").join(categoryIds);
        MapSqlParameterSource parameters = new MapSqlParameterSource();
        parameters.addValue("ids", categoryIds);
        parameters.addValue("uid", userId);
        namedJdbcTemplate.query(sql, parameters, categorySumMapper);
//        jdbcTemplate.query(sql, categorySumMapper, userId, str);
    }
